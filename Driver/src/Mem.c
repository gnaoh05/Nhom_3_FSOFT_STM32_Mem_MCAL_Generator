...
static uint8 Mem_ValidateAddressAndLength(Mem_AddressType address, Mem_LengthType length, Mem_OperationType opType) {
    ...
    for (i = 0; i < Mem_ConfigData.NumberOfBatches; i++) {
        batchStart = Mem_ConfigData.SectorBatchesPtr[i].MemStartAddress;
        batchSize  = Mem_ConfigData.SectorBatchesPtr[i].MemNumberOfSectors * 
                     Mem_ConfigData.SectorBatchesPtr[i].MemEraseSectorSize;
        ...
        if ((address >= batchStart) && (address < batchEnd)) {
            ...
            if (opType == MEM_OP_READ) {
                requiredAlignment = Mem_ConfigData.SectorBatchesPtr[i].MemMinReadSize;
            } else if (opType == MEM_OP_WRITE) {
                requiredAlignment = Mem_ConfigData.SectorBatchesPtr[i].MemWritePageSize;
            } else if (opType == MEM_OP_ERASE) {
                requiredAlignment = Mem_ConfigData.SectorBatchesPtr[i].MemEraseSectorSize;
            }
...
extern Std_ReturnType Mem_Ipw_Read(Mem_InstanceIdType instanceId, Mem_AddressType sourceAddress, Mem_DataType* destinationDataPtr, Mem_LengthType length);
extern Std_ReturnType Mem_Ipw_Write(Mem_InstanceIdType instanceId, Mem_AddressType targetAddress, const Mem_DataType* sourceDataPtr, Mem_LengthType length);
extern Std_ReturnType Mem_Ipw_Erase(Mem_InstanceIdType instanceId, Mem_AddressType targetAddress, Mem_LengthType length);
extern Std_ReturnType Mem_Ipw_BlankCheck(Mem_InstanceIdType instanceId, Mem_AddressType targetAddress, Mem_LengthType length);
